...
object WikipediaRanking {
  ...
  def containLanguage(lang: String, wikipediaArticle: WikipediaArticle): Boolean =
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int =
    rdd
      .filter(wikipediaArticle => containLanguage(lang, wikipediaArticle))
      .count()
      .toInt
  ...
  def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] =
    rdd
      .flatMap(wikipediaArticle => {
        langs
          .filter(lang => containLanguage(lang, wikipediaArticle))
          .map(lang => (lang, wikipediaArticle))
      })
      .groupByKey
