...
object TimeUsage {
  ...
  /** @return The initial data frame columns partitioned in three groups: primary needs (sleeping, eating, etc.),
    ...
    */
  private val PRIMARY_NEEDS_ACTIVITIES = List("t01", "t03", "t11", "t1801", "t1803")
  private val WORKING_ACTIVITIES = List("t05", "t1805")
  private val OTHER_ACTIVITIES = List("t02", "t04", "t06", "t07", "t08", "t09", "t10", "t12", "t13", "t14", "t15", "t16", "t18")

  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) =
    columnNames.foldLeft((List[Column](), List[Column](), List[Column]())) {
      case ((primaryNeedsColumns, workColumns, otherColumns), columnName) =>
        var newPrimaryNeedsColumns = primaryNeedsColumns
        var newWorkColumns = workColumns
        var newOtherColumns = otherColumns

        if (PRIMARY_NEEDS_ACTIVITIES.exists(columnName.startsWith)) {
          newPrimaryNeedsColumns = new Column(columnName) :: newPrimaryNeedsColumns
        } else if (WORKING_ACTIVITIES.exists(columnName.startsWith)) {
          newWorkColumns = new Column(columnName) :: workColumns
        } else if (OTHER_ACTIVITIES.exists(columnName.startsWith)) {
          newOtherColumns = new Column(columnName) :: otherColumns
        }

        (newPrimaryNeedsColumns, newWorkColumns, newOtherColumns)
    }
  ...
  def timeUsageSummary(primaryNeedsColumns: List[Column],
    ...
    val workingStatusProjection: Column =
      when($"telfs".between(1, 3), "working").otherwise("not working").as("working")
  ...
  // alternative: timeUsageSummaryDf.as[TimeUsageRow]
    timeUsageSummaryDf
      .map(row =>
        TimeUsageRow(
    ...
    summed
      ...
      .map {
        case ((age, sex, working), primaryNeeds, work, other) => TimeUsageRow(
          ...
          sex = sex,
          age = age,
          primaryNeeds = roundTo1Decimal(primaryNeeds),
          work = roundTo1Decimal(work),
